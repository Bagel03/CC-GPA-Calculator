...
export function renderResetButton() {
    ...
    const closer = createEl("button", ["btn", "btn-default", "disabled"], "Show Real Grades", { id: "resetGradesBtn" });
    footer.insertBefore(closer, footer.children[2]);
}

export function enableResetButton(classId: string) {
    const button = document.getElementById("resetGradesBtn") as HTMLButtonElement;
    if (!button.classList.contains("disabled")) return;

    button.classList.remove("disabled");

    button.addEventListener("click", () => {
        const els = document.getElementsByClassName("hypotheticalAssignment")
        for (const el of els) {
            el.remove();
        }
        recalculateGrade(classId);
        document.querySelectorAll(`[data-custom-assignment-dirty="true"]`).forEach(el => {
